  ...
  <div class="row">
   ...
   <div class="panel panel-primary col-sm-12 col-md-4">
    ...
    <div class="panel-body">
      ...
      <form method="post" action="{{ url('/enterconsultation') }}">
        ...
        <div class="form-group">
          ...
          <select id='selUser' class="form-control">
            ...
            @foreach($data as $row)
            <option  data-nhis="{{ $row->nhis_id }}"  data-patient="{{ $row->patient_id }}" data-phone="{{ $row->phone }}" data-name="{{ $row->name }}">({{ $row->patient_id }}) {{ $row->name }}</option>
            @endforeach
          </select>
        </div>
        <hr>
